...
export default function Filter({filter, onChange}: any) {

    const { name, filterType, values, label }: FilterField = filter
    const [selectedValues, setSelectedValues] = useState<any[]>(values.flatMap(item => item.isEnabled ? [item.id ? item.id : item.value] : []))
    ...
    return (
        <div className="mb-6">
            <button
                ...
                onClick={() => setOpen(o => !o)}
                aria-expanded={open}
                className="flex items-center justify-between text-xl font-bold mb-1 cursor-pointer select-none text-left"
            >
                <span>{label}</span>
    ...
    function checkbox() {
        ...
        return (
            <>
            {values.map(item =>
                    <label className="text-xl flex items-center gap-2 cursor-pointer select-none" key={i++}>
                        <input
                            type="checkbox"
...
export function normalizeFilters(data: FilterResponse, selectedParams: ProductQuery) {
    ...
    const selectedBrands: any[] = selectedParams.brands
    ...
    const normalizedBrands: FilterField[] = [{
        ...
        values: brands.map(brand => {
            const isEnabled = selectedBrands.includes(brand.id.toString())
    ...
    const normalizedAttrs: FilterField[] = attributes.map(attr => {
        return {
            ...
            values: attr.values.map(attrValue => {
                const isEnabled = selectedAttributes.find(a => a.name === attr.attribute)
                    ?.values.includes(attrValue.value.toString()) || false
                return { value: attrValue.value, count: attrValue.count, id: attrValue.id, isEnabled: isEnabled }
            })
        }
    })
